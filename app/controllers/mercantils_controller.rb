class MercantilsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @mercantil.save
        ...
        format.js { render 'layouts/create', :locals => {:@pool => @mercantil} }
      else
  ...
  def update
    ...
    respond_to do |format|
      if are_saved.all?
        ...
        format.js { render 'layouts/update', :locals => {:@pool => @mercantil} }
      else
  ...
  private
    ...
    def mercantil_params
      params.require(:mercantil).permit(:category_id, :category_type, :monto, :balance, :cuenta, :comprobante_type, :n_factura, :description, :razon_social_id, :date_of, :forma_de_pago, :forma_de_pago_nro, :receiver)
    end
end
